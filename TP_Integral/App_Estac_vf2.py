                    ...
                    if resp2 == 1:
                        ...
                        if bandera == False:
                            ...
                            if bandera == False:
                                ...
                                intervalo = (hsEstadia(verHoraIngreso(autoXPos), verHoraEgreso(autoXPos)))
...
def tarifaTotal(precio, tiempoEstadia, torre):
    if torre != 3:
        precioxMin = precio/(datetime.timedelta(minutes=60).total_seconds())*(datetime.timedelta(minutes=1).total_seconds())
        montoTotal = precioxMin * tiempoEstadia
        return montoTotal
    else:
        montoTotal = (precio*(1-0.15)) * tiempoEstadia
        return montoTotal
...
def registrarSalidaVehiculo(estacN, precioHora):
    ...
    for i in range(cantidadAutos(estacN)):
        ...
        if verPatente(autoXPos) == patente:
            ...
            tiempoEstadia=(tiempoEstadia(verHoraIngreso(autoXPos), verHoraEgreso(autoXPos)))

            monto=tarifaTotal(precioHora, tiempoEstadia, verTorre(autoXPos))
