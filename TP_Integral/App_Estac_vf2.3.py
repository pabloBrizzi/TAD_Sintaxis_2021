...
def controlEntero(numEntero):
    while isinstance(numEntero, int)==False:
        try:
            numEntero=int(numEntero)
        except:
            print("Debe ingresar un formato válido NumEntero")
            numEntero=input("Ingrese: ")
    return numEntero

def controlPrecio(precio):
    while isinstance(precio, float)==False:
        try:
            precio=float(precio)
        except:
            print("Debe ingresar un número entero o decimal")
            precio = input("->Precio: ")
...
def controlTorre(torre):
    while isinstance(torre, int)==False:
        try:
            torre=int(torre)
        except:
            print("Debe ingresar un formato de Torre valido Num. Entero")
            torre=input("->Torre: ")
    return torre
...
def ingresarVehiculo(estacN):
    ...
    torre = controlTorre(input("- Ingrese la Torre: "))
...
def ingreseMin(min):
    while isinstance(min, int)==False:
        try:
            min=int(min)
        except:
...
def controlHoraMinutos(hs,min, bandera):
        try:
...
def modificarVehiculo(estacN, precioHora):
    ...
    for i in range(cantidadAutos(estacN)):
        ...
        if verPatente(autoXPos) == patente:
            ...
            torreNueva = controlTorre(input("- Ingrese la nueva Torre: "))
            ...
            while resp == 's':
                ...
                while bandera:
                    if resp2 == 1:
                        print(" :: Hora INGRESO  ::")
                        hora_InEg = ingreseHora(input("Hora: "))
                        minu_InEg = ingreseMin(input("Min: "))
...
def eliminarVehiTorre(estacN):
    ...
    torre = controlTorre(input("-> Indique la Torre: "))
...
def generarColaxtorre(estacN):
    ...
    cola=crearCola()
    torre=controlTorre(input("-Ingrese la Torre que desea generar la cola: "))
...
precioHora = controlPrecio(input("Ingrese el Precio de la Hora: $ "))
